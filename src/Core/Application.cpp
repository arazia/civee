...
Application::Application() {
  ...
  _window = SDL_CreateWindow(settings.Title.c_str(), SDL_WINDOWPOS_CENTERED,
  ...
  if (!_context->init()) {
    std::cerr << "CRITICAL: Context failed to init!" << std::endl;
    return;
  }
...
void Application::run() {
  while (_is_running) {
    ...
    float delta_time = (float)(curr_frame_time - _last_frame_time) /
...
//   if (!_context->init()) {
//     std::cerr << "Engine Error: Failed to initialize Graphics Context." <<
...
//   "assets/shaders/Basic.frag"); if (!_shader) {
//     std::cerr << "Shader creation err." << std::endl;
//     return false;
//   }
//   _shader->bind();
//   // _shader->set_uniform_float4("u_Color", glm::vec4(0.2f, 0.8f,
//   0.3f, 1.0f));
...
//   _camera_controller =
//   std::make_shared<OrthographicCameraController>(aspect_ratio); if
//   (!_camera_controller) {
...
//   Ray ray;
//   _camera_controller->get_camera()->calculate_ray(ndc_x, ndc_y, ray.origin,
//   ray.direction);
...
//   } else if (hit_ground) {
//     std::cout << "Hovering Tile: " << hovered_hex.q << ", " << hovered_hex.r
//     << std::endl;
//   }

//   // ImGui Rendering
//   Uint64 curr_frame_time = SDL_GetPerformanceCounter();
//   _delta_time = (float)(curr_frame_time - _last_frame_time) / (float)
//   SDL_GetPerformanceFrequency(); _last_frame_time = curr_frame_time;
...
//       ImGui::GetIO().Framerate, ImGui::GetIO().Framerate); ImGui::Text("Delta
//       Time: %.4f", _delta_time); ImGui::Text("Coords: %.1f %.1f %.1f", pos.x,
//       pos.y, pos.z); ImGui::End();
//   }
...
//   // RenderCommand::set_clear_color({color[0], color[1], color[2],
//   color[3]});
