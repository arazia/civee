...
OrthographicCamera::OrthographicCamera(float left, float right, float bottom,
    ...
    : _projection_matrix(glm::ortho(left, right, bottom, top, -100.0f, 100.0f)),
      _view_matrix(1.0f), _left(left), _right(right), _bottom(bottom),
      _top(top) {
...
void OrthographicCamera::recalculate_view_matrix() {
  ...
  // transform = glm::rotate(transform, glm::radians(_rotation.z), glm::vec3(0,
  // 0, 1)); transform = glm::rotate(transform, glm::radians(_rotation.y),
  // glm::vec3(0, 1, 0)); transform = glm::rotate(transform,
  // glm::radians(_rotation.x), glm::vec3(1, 0, 0));

  transform =
      glm::rotate(transform, glm::radians(_rotation.y), glm::vec3(0, 1, 0));
  transform =
      glm::rotate(transform, glm::radians(_rotation.x), glm::vec3(1, 0, 0));
  transform =
      glm::rotate(transform, glm::radians(_rotation.z), glm::vec3(0, 0, 1));
...
void OrthographicCamera::calculate_ray(float ndc_x, float ndc_y,
                                       glm::vec3 &out_origin,
