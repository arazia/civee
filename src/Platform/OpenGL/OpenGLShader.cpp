...
OpenGLShader::OpenGLShader(const std::string &vertex_path, const std::string &frag_path) {
...
void OpenGLShader::set_uniform_float3(const std::string& name, const glm::vec3& value) {
  glUniform3f(get_uniform_location(name), value.x, value.y, value.z);  
}

void OpenGLShader::set_uniform_float4(const std::string& name, const glm::vec4& value) {
...
int OpenGLShader::get_uniform_location(const std::string &key) {
  if (_uniform_location_cache.find(key) != _uniform_location_cache.end())
    ...
    if (location == -1) {
        std::cout << "Uniform key err: " << key << ", Renderer id: " << _renderer_id <<  std::endl;
    }
...
uint32_t OpenGLShader::compile_shader(uint32_t type, const std::string &source) {
...
uint32_t OpenGLShader::create_program(const std::string &vertex_src, const std::string &frag_src) {
  ...
  if (err != GL_NO_ERROR) std::cerr << "GL Error after Attach VS: " << err << std::endl;
  glAttachShader(program, fs);
  if (err != GL_NO_ERROR) std::cerr << "GL Error after Attach FS: " << err << std::endl;
  ...
  if (isLinked == GL_FALSE) {
    ...
    if (maxLength > 0) {
            ...
            std::cerr << "Shader Link Error: (No details provided by driver)" << std::endl;
        }
    return 0;
  }
